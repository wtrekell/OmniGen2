...
#     def _precompute_freqs_cis(self, axes_dim: Tuple[int, int, int], axes_lens: Tuple[int, int, int], theta: int) -> List[torch.Tensor]:
#         freqs_cis = []
...
# def get_2d_rotary_pos_embed_omnigen2(axes_dim, axes_lens, theta: int = 10000):
#     freqs_cis = []
#     freqs_dtype = torch.float32 if torch.backends.mps.is_available() else torch.float64
#     for i, (d, e) in enumerate(zip(axes_dim, axes_lens)):
